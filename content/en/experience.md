# Roles and Experience

## Academic Positions

- Siberian Federal University — Head of the Software Engineering Department.
- Siberian Federal University — Professor at the Department of Information Technologies in Creative and Cultural Industries.

## Project Roles

- AirScope — direction lead / scientific and technical project lead.
- Siberiana — project lead of the digital cultural heritage platform for the Angara-Yenisei region.
- Applied R&D and software development — formulation of scientific and technical tasks, product architecture vision, development coordination, and preparation of results for deployment.

## Competencies

- Management and modeling of heterogeneous distributed computing systems.
- Multi-agent systems, decision support, and resource optimization.
- Software engineering and software development.
- Artificial intelligence, neural networks, and machine learning.
- Digital humanities, AR/VR, and cultural heritage platforms.
- Educational program design and e-learning.

## Leadership Scope

Oleslav Antamoshkin combines research, education, and project work: he organizes research and engineering teams, and leads technical specifications, architectural decisions, budgets, schedules, and reporting.

In the Siberiana and AirScope projects, he coordinated Siberian Federal University interaction with external organizations, translated research groundwork into working digital products, and prepared pilot scenarios for customers.

## Education

- Siberian State Aerospace University: Bachelor of Engineering and Technology, 2000.
- Siberian State Aerospace University: Master of Engineering and Technology in Systems Analysis and Control, 2003.
- Academic degrees: Candidate of Engineering Sciences, 2006; Doctor of Engineering Sciences, 2017.
- Specialty: Systems Analysis, Control, and Information Processing.
- Academic title: Associate Professor.

## Professional Development

Between 2010 and 2025, 58 professional development programs were completed. The main areas were software engineering, artificial intelligence, digital education technologies, project management, information security, and digital humanities practices.

## Personal Interests

- Siberian history and cultural heritage.
- Travel, routes, and field observation across the region.
- Maps, spatial data, visual materials, and digital archives.
- Experimenting with new digital tools for research and engineering tasks.
